...
export function TicketTable({
      ...
      <table className="min-w-full">
        <thead>
          <tr className="border-b border-gray-500">
            <th className="px-7 py-4 text-left text-xs font-semibold text-gray-700 uppercase">
              Ticket
            </th>
            <th className="px-7 py-4 text-left text-xs font-semibold text-gray-700 uppercase">
              Status
            ...
            <th className="px-7 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider hidden sm:table-cell">
              Assigned
            </th>
            <th className="px-7 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider hidden md:table-cell">
              Created
            </th>
            <th className="px-7 py-4 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">
              Actions
            </th>
          </tr>
        </thead>
              ...
              <td className="px-6 py-4">
                <div className="flex items-start space-x-3">
                  {unreadForUser(ticket) && (
                    <div className="w-2 h-2 bg-blue-600 rounded-full mt-2 animate-pulse shrink-0" />
                  )}
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-6 h-6 bg-gray-200 rounded-full flex items-center justify-center">
                    <span className="text-xs font-medium text-gray-600">
                      {assignedLabel(ticket) === "Unassigned"
                        ? "U"
                        : assignedLabel(ticket)[0]}
                    </span>
                  </div>
              ...
              <td className="px-6 py-4">
                <div className="flex items-center justify-end space-x-2">
                  <Link
                    ...
                    className="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-800 rounded-lg text-xs font-medium hover:bg-blue-100 transition-colors"
                  >
                    <svg
                  ...
                  {mode === "admin" && (
                    <button
                      ...
                      className="inline-flex items-center px-3 py-1.5 bg-red-50 text-red-800 rounded-lg text-xs font-medium hover:bg-red-100 transition-colors"
                    >
                      <svg
                        className="w-3 h-3 mr-1"
                        fill="none"
  ...
  const renderCardView = () => (
    <div className="grid gap-6 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-1">
      {tickets.map((ticket) => (
        <div
          key={ticket._id}
          className="bg-gray-50 border border-gray-200 rounded-xl shadow-sm hover:shadow-lg transition-all duration-200 p-6 group"
        >
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-700 mb-6 max-w-sm mx-auto">
            Get started by creating your first support ticket to track and
            resolve issues.
          </p>
          {mode === "client" && (
            <Link
              href="/client/tickets/create"
              className="inline-flex items-center px-5 py-2 bg-blue-500 text-white rounded-xl font-medium hover:bg-purple-500 transition-colors shadow-sm"
            >
              Create Ticket
            </Link>
          )}
      ...
      {/* Pagination */}
      {tickets.length > 0 && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-6 py-4 rounded-xl border border-gray-200 space-y-3 sm:space-y-0">
            ...
            <nav aria-label="Page navigation example">
              <ul className="flex -space-x-px text-sm">
                <li>
                  <a
                    href="#"
                    className="flex items-center justify-center text-body bg-neutral-secondary-medium hover:bg-neutral-tertiary-medium hover:text-heading font-medium rounded-s-base text-sm px-3 h-9 focus:outline-none"
                  >
                    Previous
                  </a>
                </li>
                ...
                <li>
                  <a
                    href="#"
                    className="flex items-center justify-center text-body  hover:bg-neutral-tertiary-medium hover:text-heading font-medium rounded-e-base text-sm px-3 h-9 focus:outline-none"
                  >
                    Next
                  </a>
                </li>
          ...
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to delete "{deleteModal.ticket?.title}"?
